...
final class GatewayRateLimitFilter implements GlobalFilter, Ordered {
    ...
    GatewayRateLimitFilter(@Value("${app.rate-limit-per-minute:120}") int limit) { this.limit = limit; }
    ...
    @Override public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        Window window = windows.compute(key, (ignored, current) ->
                current == null || current.minute() != minute ? new Window(minute, new AtomicInteger(1))
