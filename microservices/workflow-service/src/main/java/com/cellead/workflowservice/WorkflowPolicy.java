...
class WorkflowPolicy {
    ...
    void authorizeView(WorkflowRequest workflow, AuthenticatedUser user) {
        if (user == null || !("ADMIN".equals(user.role())
                || workflow.submitterId.equals(user.id()) || workflow.approverId.equals(user.id()))) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Workflow is not visible to this user");
        }
    }
    ...
    void authorizeCancellation(WorkflowRequest workflow, AuthenticatedUser user) {
        ...
        if (!workflow.submitterId.equals(user.id())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Only the submitter can cancel this workflow");
        }
