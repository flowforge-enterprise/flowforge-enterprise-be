...
class WorkflowQueryService {
  ...
  List<TimelineEvent> timeline(WorkflowRequest workflow) {
    ...
    events.add(
        new TimelineEvent(
            "SUBMITTED", workflow.submitterUsername, "Workflow submitted", workflow.createdAt));
    approvals
        .findByWorkflowIdOrderByCreatedAtDesc(workflow.id)
        .forEach(
            approval ->
                events.add(
                    new TimelineEvent(
    ...
    if (workflow.status == WorkflowStatus.CANCELLED) {
      events.add(
          new TimelineEvent(
              "CANCELLED", workflow.submitterUsername, "Workflow cancelled", workflow.updatedAt));
    }
  ...
  WorkflowStats statistics() {
    return new WorkflowStats(
        workflows.count(),
        workflows.countByStatus(WorkflowStatus.PENDING),
        workflows.countByStatus(WorkflowStatus.APPROVED),
        workflows.countByStatus(WorkflowStatus.REJECTED),
