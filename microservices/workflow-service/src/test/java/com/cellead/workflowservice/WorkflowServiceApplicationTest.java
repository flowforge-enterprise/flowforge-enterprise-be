...
class WorkflowServiceApplicationTest {
  ...
  @Test
  void domainEventsAreDurablyQueuedBeforeDelivery() {
    dispatcher.publish(
        new DomainEvent(
            "TEST_EVENT",
            99L,
    ...
    OutboxEvent saved =
        outbox.findAll().stream()
            .filter(e -> "TEST_EVENT".equals(e.eventType))
            .findFirst()
            .orElseThrow();
  ...
  @Test
  void workflowRepositorySupportsFinalStateStatistics() {
    WorkflowRequest request =
        new WorkflowRequest(
            new CreateRequest("Test", "Description", "GENERAL", Priority.MEDIUM, 2L, "approver"),
