...
class AccountSecurityPolicy {
  ...
  LoginEligibility eligibility(AppUser user) {
    if (!user.active) {
      return new LoginEligibility(false, "DISABLED");
    }
    if (user.lockedUntil != null && user.lockedUntil.isAfter(now())) {
      return new LoginEligibility(false, "LOCKED");
    }
  ...
  void recordFailure(AppUser user) {
    ...
    if (user.failedLoginAttempts >= maxFailedAttempts) {
      user.lockedUntil = now().plusSeconds(lockSeconds);
    }
  }
  ...
  void validatePasswordChange(AppUser user, boolean currentMatches, boolean newMatchesCurrent) {
    ...
    if (!user.active) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Disabled account cannot change password");
    }
  }
