...
class AuthApplicationTest {
    ...
    @Test void loginAndSecurityContractWork() throws Exception {
    ...
    @Test void refreshTokenCanRotateButCannotAccessProtectedApis() throws Exception {
        String body=mvc.perform(post("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"username\":\"requester\",\"password\":\"password123\"}"))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        String refresh=mapper.readTree(body).path("refreshToken").asText();
        mvc.perform(get("/api/auth/me").header("Authorization","Bearer "+refresh)).andExpect(status().isUnauthorized());
